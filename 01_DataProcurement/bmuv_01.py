...
LIST_URL = f'{BASE_URL}/presse/pressearchiv'
...
class BMUVSpider(Spider):
    ...
    def parse_article(self, response: Response, **kwargs: Any):
        ...
        topic = None
        for row in content.css('.c-meta-facts__row'):
            label = get_all_stripped(row, 'strong *::text')
            if label == 'Thema:':
                topic = get_all_stripped(row, 'div > span *::text')
